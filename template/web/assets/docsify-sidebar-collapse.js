(function (global, factory) {
  ...
  function handleMenuClick(e) {
    // 1. 全局阻止冒泡：无论点击哪里（箭头、文字、空白），都阻止事件传给 Docsify
    // 这样能确保在手机上点击侧边栏时，侧边栏不会自动关闭（"自动收起来"）。
    e.stopPropagation();
    ...
    // Detect if the click target is a Link (A tag)
    var isLink = e.target.tagName === 'A' || findTagParent(e.target, 'A', 2);

    if (isLink) {
        // If it's a link click, we only expand, never collapse.
        // We also don't want to toggle if it's already open.
        if (!newActiveNode.classList.contains('open')) {
            newActiveNode.classList.add('open');
            newActiveNode.classList.remove('collapse');
            // Ensure parents are open too
            openActiveToRoot(newActiveNode);
        }
    } else {
        // If it's NOT a link click, it could be the arrow OR the padding/empty space.
        // We want ONLY the arrow to toggle (collapse).
        // The arrow is positioned to the left of the LI (left: -20px).
        // So we check if the click x-coordinate is strictly to the left of the LI's content box.
        var rect = newActiveNode.getBoundingClientRect();
        var isArrowClick = e.clientX < rect.left;

        if (isArrowClick) {
            // Toggle logic for Arrow
            var isEffectivelyOpen = newActiveNode.classList.contains('open') || 
                                   (newActiveNode.classList.contains('active') && !newActiveNode.classList.contains('collapse'));
            ...
            // Clicked on padding/empty space (not arrow, not link).
            // User request: "Don't automatically collapse".
            // So we treat this like a Link click: Expand only (or do nothing if already open).
            if (!newActiveNode.classList.contains('open')) {
                newActiveNode.classList.add('open');
                newActiveNode.classList.remove('collapse');
                openActiveToRoot(newActiveNode);
  ...
  function addLevelClass() {
    function find(root, level, isLibraryContext) {
      root && root.childNodes && root.childNodes.forEach(function (child) {
        if (child.classList && child.classList.contains('folder')) {
          child.classList.add("level-".concat(level));

          // Check if this node is the "Library" node
          var isLibrary = false;
          if (level === 1) {
             var link = child.querySelector('a');
             if (link && link.innerText.indexOf('通用知识库') !== -1) {
                 isLibrary = true;
             }
          }
          
          // Determine if we are in Library context (either this is Library, or parent was)
          var currentIsLibraryContext = isLibraryContext || isLibrary;

          // Custom logic: If in Library context, expand one level deeper (Level 2)
          var effectiveDisplayLevel = window.$docsify.sidebarDisplayLevel;
          if (currentIsLibraryContext) {
              effectiveDisplayLevel = (window.$docsify.sidebarDisplayLevel || 0) + 1;
          }
          
          // --- FIX START: Diary default collapse ---
          var isDiary = false;
          var link = child.querySelector('a');
          // Check if this LI is the "Diary" node
          if (link) {
             // Check href for 'diary' or text content
             var href = link.getAttribute('href');
             var text = link.innerText;
             if ((href && href.indexOf('diary') !== -1) || (text && text.indexOf('日记') !== -1)) {
                 isDiary = true;
             }
          }
          
          var shouldOpen = (window.$docsify && window.$docsify.sidebarDisplayLevel && typeof window.$docsify.sidebarDisplayLevel === 'number' && level <= effectiveDisplayLevel);
          
          // If it's Diary, force it closed.
          // Note: If the user is IN the diary folder, openActiveToRoot will handle opening it.
          // This logic primarily controls the "default" expansion state.
          if (isDiary) {
              shouldOpen = false;
          }
          // --- FIX END ---
          ...
          if (child && child.childNodes.length > 1) {
            find(child.childNodes[1], level + 1, currentIsLibraryContext);
          }
        }
      });
    }

    find(document.querySelector('.sidebar-nav > ul'), 1, false);
  }
