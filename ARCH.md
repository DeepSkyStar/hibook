...
* **`hibook.py`**: 纯粹的入口与命令行解析中心，不做具体业务。
* **`hi_server.py`**: 单纯负责基于 `socketserver` 开启 HTTP 环境，向前端提供由底层 Shell / Git 衍生的纯 REST API 与网络静态资源服务层。
* **`hi_export.py`**: 负责系统级的离线导出及硬编排能力。
* **`template/web/assets/`**: 前端的实体承载区，前端逻辑彻底插件化并收敛于独立的 `.js` 中，绝不在 `.html` 里写入大段脏逻辑。
